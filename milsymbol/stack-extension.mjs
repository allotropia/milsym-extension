// Adds the functionality to milsymbol to add stacks of symbols
// Simply add the 'stack' property to your options with a value
// in the range from 0 to inf.
export default function stack(ms) {
  let modifier;
  const drawArray1 = [];
  const drawArray2 = [];
  ...
  const stackOutline = [];
  const stack = [];
  for (let i = this.options.stack; i >= 1; i--) {
    stackOutline.push({
      type: "translate",
      x: 15 * i,
      y: 9 * i,
  ...
  const bbox = ms.BBox(this.metadata.baseGeometry.bbox);
  bbox.x2 += 15 * this.options.stack;
  bbox.y2 += 10 * this.options.stack;
