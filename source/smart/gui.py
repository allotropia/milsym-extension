        ...
        try:
            if self.tree_control is not None:
                ...
                try:
                    ...
                    drag_source = toolkit.getDragSource(peer)
        ...
        try:
            ...
            if diagram and hasattr(diagram, 'move_tree_item'):
                ...
                if success:
                    ...
                    self.refresh_tree()
                    print(f"Successfully moved {source_node_name} relative to {target_node_name}")
                    return True
                else:
...
class TreeMouseHandler(unohelper.Base, XMouseListener):
    ...
    def mousePressed(self, event):
        ...
        # Could be used to start drag operation timing
        pass
    ...
    def mouseEntered(self, event):
        ...
        try:
            # Show drag cursor when hovering over draggable items
            tree_control = self.dialog_handler.tree_control
            if tree_control:
                node = tree_control.getNodeForLocation(event.X, event.Y)
                if node and hasattr(node, 'getDisplayValue'):
                    node_name = node.getDisplayValue()
                    # Don't show drag cursor for root node
                    if "Root" not in node_name and "Diagram Structure" not in node_name:
                        # Could set custom cursor here if supported
                        pass
        except Exception as e:
...
class TreeNodeTransferable(unohelper.Base, XTransferable):
    ...
    def getTransferData(self, flavor):
        ...
        if self.isDataFlavorSupported(flavor):
            ...
            return uno.Any("string", self.node_name)
        return None
